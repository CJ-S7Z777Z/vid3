...
REGULAR_DAILY_LIMIT = int(os.getenv("REGULAR_DAILY_LIMIT"))  # Значение по умолчанию, например 10
ADMIN_DAILY_LIMIT = int(os.getenv("ADMIN_DAILY_LIMIT"))    # Значение по умолчанию, например 100
...
BOT_TOKEN = os.getenv("BOT_TOKEN")
   ...
   # Логин и пароль для Instagram
INSTAGRAM_USERNAME = os.getenv("INSTAGRAM_USERNAME")
INSTAGRAM_PASSWORD = os.getenv("INSTAGRAM_PASSWORD")

   # Путь к файлу куки для Instagram, если используется
       ...
       if any(
           ...
           try:
               ...
               ydl_options = {
                   ...
                   "fragment_retries": 3,
                   "retry_sleep_seconds": 5,  # Задержка между попытками
                   "username": INSTAGRAM_USERNAME,  # Добавляем логин
                   "password": INSTAGRAM_PASSWORD,  # Добавляем пароль
               }

               # Проверяем, является ли ссылка Instagram
               if "instagram.com" in url:
                   if os.path.exists(INSTAGRAM_COOKIE_FILE):
                       ydl_options["cookiefile"] = INSTAGRAM_COOKIE_FILE
                   else:
                       await send_message_with_retry(
                           update,
                           "❌ Не удалось найти файл куки для Instagram. Пожалуйста, установите переменную INSTAGRAM_COOKIE_FILE с путем к файлу куки, или используйте логин и пароль.",
                       )
                       await loading_message.delete()
                       return

               max_retries = ydl_options.get("retries", 3)
               delay = ydl_options.get("retry_sleep_seconds", 5)
                   ...
                   try:
                       ...
                       if "429" in error_message:
                           if attempt < max_retries:
                               await send_message_with_retry(
                                   update,
                                   f"❌ Слишком много запросов к Instagram. Повторная попытка через {delay} секунд..."
                               )
                               ...
                               await send_message_with_retry(
                                   update,
                                   "❌ Не удалось скачать видео из-за превышения лимита запросов. Пожалуйста, попробуйте позже."
                               )
                               await loading_message.delete()
                               return
                       else:
...
def main():
       ...
       # Проверяем наличие файла куки для Instagram
       if any("instagram.com" in tariff["name"] for tariff in tariffs):
           if not os.path.exists(INSTAGRAM_COOKIE_FILE):
               logging.warning(f"Файл куки для Instagram не найден по пути: {INSTAGRAM_COOKIE_FILE}. Для скачивания из Instagram он обязателен либо укажите логин и пароль.")
