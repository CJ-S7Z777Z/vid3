...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in ADMIN_CHAT_IDS or is_admin(chat_id):
        ...
        await send_message_with_retry(
            update,
            "Привет! Я помогу скачивать видео из TikTok, YouTube и VK.",
        ...
        await send_message_with_retry(
            update,
            "Отправьте ссылку на видео из TikTok, YouTube или VK, и я скачаю его для вас.",
        )
...
async def handle_user_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if any(
        domain in url
        for domain in ["tiktok.com", "youtube.com", "youtu.be", "vk.com", "vk.ru"]
    ):
        ...
        try:
            ...
            # Генерируем уникальное имя файла
            max_retries = 1  # Убраны повторные попытки
            ...
            for attempt in range(1, max_retries + 1):
                try:
                    ...
                    await send_message_with_retry(
                        update,
                        f"❌ Ошибка при скачивании видео: {error_message}"
                    )
        ...
        await send_message_with_retry(
            update,
            "⚠️ Пожалуйста, отправьте ссылку на видео из TikTok, YouTube или VK.",
        )
